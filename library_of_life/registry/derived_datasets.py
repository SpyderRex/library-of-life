...
class DerivedDatasets:
    ...
    def create_new_derived_dataset(
        self, username=None, password=None, derived_dataset=None
    ):
        """
        ...
        if self.auth_type == "basic":
            ...
            return hc.post_with_auth_and_json(
                base_url + self.endpoint, auth=auth, json=derived_dataset
            )
        else:  # OAuth
            headers = self.auth_headers
            return hc.post_with_auth_and_json(
                base_url + self.endpoint, headers=headers, json=derived_dataset
            )
    ...
    def update_derived_dataset(
        self, doi_prefix, doi_suffix, username=None, password=None, data=None
    ):
        """
        ...
        if self.auth_type == "basic":
            auth = (username, password)
            return hc.post_with_auth_and_json(
                base_url + self.endpoint + resource, auth=auth, json=data
            )
        else:  # OAuth
            headers = self.auth_headers
            return hc.post_with_auth_and_json(
                base_url + self.endpoint + resource, headers=headers, json=data
            )

    def get_derived_datasets_by_key(
        self, key, limit: Optional[int] = None, offset: Optional[int] = None
    ):
        """
    ...
    def get_derived_datasets_by_doi(
        self,
        doi_prefix,
        doi_suffix,
    ...
    def get_derived_datasets_by_user(
        self, user, limit: Optional[int] = None, offset: Optional[int] = None
    ):
        """
    ...
    def get_derived_dataset_citation(
        self,
        doi_prefix,
        doi_suffix,
        limit: Optional[int] = None,
    ...
    def get_derived_dataset_related_datasets(
        self,
        doi_prefix,
        doi_suffix,
        limit: Optional[int] = None,
