...
class Networks:
    """
    A class for interacting with the networks section of the Registry API.
    
    Attributes:
        endpoint: The endpoint for this section of the API.
    """
    def __init__(self, use_caching=False, cache_name="networks_cache", backend="sqlite", expire_after=3600):
        self.endpoint = "network"
          
        if use_caching:
            requests_cache.install_cache(cache_name, backend=backend, expire_after=expire_after)
     
    def list_all_networks(self, identifier_type: Optional[str]=None,
        ...
        """
        Returns a list of all current networks (deleted networks are not listed).
        ...
        Returns:
            dict: A dictionary containing a list of networks.
        """
        params: Dict[str, Any] = {}
        params_list = [
    ...
    def get_network_by_key(self, key):
        """
        Returns details of a single network matching the given key.
        
        Args:
            key (str): The key of the entity (dataset, organization, network etc.).
        
        Returns:
            dict: Returns details of a single network.
        """
        resource = f"/{key}"
        try:
